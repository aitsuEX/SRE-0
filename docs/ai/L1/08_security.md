...
## Room/Roster/Incident State Persistence

`lib/room-registry.ts` and `lib/incident/state.ts` store rooms, rosters, and
incident state in a `Map()` attached to `globalThis`, scoped to a single
Node.js process. This is:

- **Process-local, not shared/persistent.** State does not survive a process
  restart and is not visible to any other process.
- **Safe for a single long-lived container** (e.g. the `Dockerfile` image run
  as one instance) where every request lands on the same process.
- **NOT serverless-safe** for a horizontally-scaled deployment (e.g. Vercel
  functions that can cold-start or scale across multiple instances). Two
  requests for the same `channel` can land on different instances, each with
  its own empty registry — this would reintroduce roster/attribution bugs
  that look identical to the transcript speaker-attribution bug but are
  actually caused by split state, not by the Agora event pipeline.

Do not claim production scalability for multi-instance deployments unless
this is replaced with a shared store (e.g. Redis) — that is an explicit
architecture change, not a bug fix, and should not be done silently.

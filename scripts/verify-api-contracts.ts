...
async function verifyInviteAgentSuccess() {
  ...
  try {
    ...
    assert(
      ...
      'POST /api/invite-agent must subscribe the agent to every participant ' +
        "(remoteUids: ['*']), not just the requester — scoping to a single " +
        'uid is the multi-user transcript attribution bug.',
    );
  } finally {
